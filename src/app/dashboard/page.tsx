      ...
      <aside className={`w-full md:w-80 lg:w-96 border-r border-zinc-800 flex flex-col flex-shrink-0 ${mobileView === "detail" ? "hidden md:flex" : "flex"} md:flex`}>
        <div className="relative h-12 border-b border-zinc-800 flex items-center px-4 sm:px-5 justify-between flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            {unreadCount > 0 && <span className="text-[10px] font-medium bg-indigo-500/20 text-indigo-400 px-2 py-0.5 rounded-full">{unreadCount}</span>}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {initialLoading && messages.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 text-zinc-600">
              ...
              <p className="text-xs text-zinc-700 mt-1">Waiting for incoming mail...</p>
            </div>
          ) : sortedMessages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 text-zinc-600">
              <Mail className="w-8 h-8 mb-2 opacity-30" />
              <p className="text-xs">{searchQuery ? "Aucun message trouvé pour cette recherche" : "No messages yet"}</p>
            </div>
          ) : (
            sortedMessages.map((msg) => (
              <button key={msg.id} onClick={() => handleSelectMessage(msg)}
                className={`w-full text-left px-4 py-3.5 md:py-3 border-b border-zinc-800/50 transition-colors hover:bg-zinc-900/80 ${selectedMsg?.id === msg.id ? "bg-zinc-900/80" : ""}`}>
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 flex-shrink-0">
                    {msg.seen ? <MailOpen className="w-4 h-4 text-zinc-600" /> : <Mail className="w-4 h-4 text-indigo-400" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] text-zinc-600 flex-shrink-0">{formatTime(msg.createdAt)}</span>
                    </div>
                    <p className={`text-xs truncate mt-0.5 ${msg.seen ? "text-zinc-600" : "text-zinc-400"}`}>{msg.subject}</p>
                    <p className="text-[11px] text-zinc-600 truncate mt-0.5">{msg.intro}</p>
                  </div>
                </div>
              </button>
            ))
      ...
      <main className={`flex-1 flex flex-col min-w-0 ${mobileView === "inbox" ? "hidden md:flex" : "flex"} md:flex`}>
        {loadingDetail ? (
          ...
          <div className="flex-1 flex flex-col min-h-0 animate-fade-in-up">
            <div className="border-b border-zinc-800 px-5 sm:px-7 py-4 flex-shrink-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2.5 mt-1">
                    <div className="w-6 h-6 rounded-full bg-zinc-800 flex items-center justify-center flex-shrink-0">
                      <span className="text-[10px] font-medium text-zinc-400">{(selectedMsg.from.name || selectedMsg.from.address)[0]?.toUpperCase()}</span>
                    </div>
                    <div className="min-w-0">
                      <span className="text-xs text-zinc-300">{selectedMsg.from.name || selectedMsg.from.address}</span>
                      <span className="text-xs text-zinc-600 ml-1.5 hidden sm:inline">&lt;{selectedMsg.from.address}&gt;</span>
                    </div>
                  </div>
                  <p className="text-[11px] text-zinc-600 mt-1.5">To: {selectedMsg.to.map((t) => t.address).join(", ")}</p>
                  <p className="text-[11px] text-zinc-600">{new Date(selectedMsg.createdAt).toLocaleString()}</p>
                </div>
            ...
            {selectedMsg.attachments?.length > 0 && (
              <div className="border-b border-zinc-800 px-5 sm:px-7 py-3 flex-shrink-0">
                <p className="text-[11px] text-zinc-600 uppercase tracking-wider mb-2">Attachments ({selectedMsg.attachments.length})</p>
                <div className="flex flex-wrap gap-2">
                  {selectedMsg.attachments.map((att) => (
                    <div key={att.id} className="flex items-center gap-1.5 h-7 px-2.5 bg-zinc-900 border border-zinc-800 rounded-md text-[11px] text-zinc-400">
      ...
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-40 pointer-events-none">
        <div className="border border-zinc-800/80 bg-zinc-900/95 text-zinc-400 text-xs font-mono px-4 py-2 rounded-lg shadow-lg shadow-black/30 flex items-center gap-4 backdrop-blur-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-500">Copier l&apos;e-mail</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-500">Rafra&icirc;chir</span>
          </div>
        </div>
      </div>
    </div>
