...
const FEATURES = [
  {
    ...
    desc: "Generate a temporary email address in seconds. No signup, no commitment, no trace.",
  },
  {
    icon: Shield,
    title: "Privacy First",
    desc: "Your real inbox stays untouched. All data lives in your browser, nothing stored server-side.",
  },
  {
    icon: Clock,
    title: "Auto-Refresh",
    desc: "Live Mercure SSE connection delivers messages in real-time. Fallback polling every 30s.",
  },
  {
    icon: Globe,
    title: "Multiple Domains",
    desc: "Choose from available domains. Create and manage multiple accounts effortlessly.",
  },
];
...
const STEPS = [
  {
    num: "01",
    icon: Terminal,
    title: "Generate or Login",
    desc: "Create a new temporary address or sign into an existing one with a single click.",
  },
  {
    num: "02",
    icon: Inbox,
    title: "Receive Messages",
    desc: "Share your new address. Emails arrive instantly in your AetherFetch inbox.",
  },
  {
    num: "03",
    icon: Eye,
    title: "Read & Manage",
    desc: "View, search, copy, and delete messages. Full control over your temporary mailbox.",
  },
];

const FAQ_ITEMS = [
  {
    q: "What is AetherFetch?",
    a: "AetherFetch is a temporary email service that lets you create disposable email addresses for quick signups, verifications, and privacy protection. No personal data required.",
  },
  {
    q: "Are my messages stored on a server?",
    a: "No. Your session data and messages are stored locally in your browser via localStorage. Nothing is persisted on our servers beyond what the mail.tm API requires for delivery.",
  },
  {
    q: "How long do messages stay?",
    a: "Messages persist as long as your session is active. When you clear your browser data or your session expires, the mailbox and all messages are gone.",
  },
  {
    q: "Can I use this for production email?",
    a: "No. AetherFetch is designed for temporary, disposable use. Do not rely on it for important or long-term communication.",
  },
];
...
export default function LandingContent() {
  return (
    <div className="flex-1 overflow-y-auto">
      {/* Hero */}
      <section className="relative min-h-[100dvh] flex items-center justify-center px-4 pt-24 pb-16">
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded-full text-xs text-zinc-400 mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/register"
              className="group h-11 px-7 flex items-center gap-2 bg-indigo-500 hover:bg-indigo-400 text-white text-sm font-medium rounded-lg transition-all duration-200 hover:shadow-lg hover:shadow-indigo-500/20 active:scale-[0.98]"
            >
              Get Started
            ...
            <Link
              href="/login"
              className="h-11 px-7 flex items-center gap-2 text-sm font-medium text-zinc-300 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 rounded-lg transition-all duration-200"
            >
              I have an account
            </Link>
          </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {FEATURES.map((f) => (
              <div
                key={f.title}
                className="group p-6 bg-zinc-900 border border-zinc-800 rounded-xl hover:border-zinc-700 transition-all duration-300 hover:shadow-xl hover:shadow-black/20"
              >
